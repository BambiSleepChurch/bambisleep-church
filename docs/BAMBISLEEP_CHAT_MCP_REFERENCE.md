...
> Trigger detection, TTS processing, chat management, and session handling for BambiSleep experiences.
...
```json
{
  ...
  "triggers": [
    { "id": "bambi", "name": "Bambi", "category": "primary" }
  ]
}
```
...
```javascript
// 1. Create session
const session = await fetch('/api/bambisleep-chat/session', { method: 'POST' })
  .then(r => r.json());
